# Building MintCap (developer guide)

This is the technical/developer documentation. If you just want to *use* the addon, see the
[README](README.md).

MintCap is a single-DLL [ReShade](https://reshade.me) addon written in C++17, Windows x64 only.
The output is `MintCap.addon64`. All logic lives in [`src/main.cpp`](src/main.cpp).
...
The version comes from the git tag and nothing else. CI passes `-DMINTCAP_VERSION=X.Y.Z`, which flows
...
`MINTCAP_VERSION_STR` compile definition the overlay prints, and the release zip's filename. Local and
...
```sh
cmake -S . -B build -G "Visual Studio 17 2022" -A x64 -DMINTCAP_VERSION=1.0.0
```
...
default in 1.92; at 1.90.4 it is `void*`, and ReShade passes `resource_view` handles (`uint64_t`)
through `ImTextureID`, so its overlay header opens with a `static_assert` demanding exactly this
define. The addon draws no images itself, but the define is still mandatory — without it the build
fails on the first `#include <reshade.hpp>`.

Everything else is Windows system libraries: `winmm` (timer granularity) and `shell32` (open links).
...
- **Frame cap** - in the `reshade::addon_event::present` callback we measure time since the last
  present and, when a cap is set, block until one `1/cap` second interval has elapsed: sleep until ~1 ms before
  ...
  at load/unload.
- **Persistence** - the cap is read in `init_effect_runtime` and written on change via
  `reshade::get/set_config_value` under the `[MintCap]` config section, key `FpsCap`. It is
  clamped to `[0, kMaxFps]` on the way in as well as on the way out: `ReShade.ini` is plain text a
  user can hand-edit, and a negative or absurd value must never reach the pacing loop.
...
- **Logging** - `mc_log()` wraps `reshade::log_message`, so everything lands in `ReShade.log`
...
3. **Zip** `dxgi.dll` + `MintCap.addon64` + `packaging/Enable-ReShade.bat` +
...
```sh
gh release view vX.Y.Z --repo Nipeno/MintCap --json assets
```
...
Vendored headers and the bundled binary keep their upstream notices under `third_party/`. All
three ship inside the release zip:
...
- [`third_party/THIRD-PARTY-NOTICES.txt`](third_party/THIRD-PARTY-NOTICES.txt) - the inventory:
  what is inside `MintCap.addon64` versus inside `dxgi.dll`, and every project ReShade statically
  links, with its licence and upstream URL.

**Why the third file exists.** BSD 3-Clause and MIT both require their notice to travel with
binary redistributions, and we redistribute ReShade's binary - so ReShade's *own* dependencies
(MinHook, stb, glad, utfcpp, SPIRV-Headers, VMA, fpng, DirectX-Headers, OpenXR, OpenVR,
simple-lossless-encoder) have notice requirements that land on us, not on upstream. Upstream's
`LICENSE.md` is plain BSD-3 with no third-party section, so the inventory is ours to keep. If the
bundled ReShade version ever changes what it links, this file is what needs updating.

## License

MintCap itself is **source available, not open source** - see [LICENSE](LICENSE). In short: free
to use, free to read, free to build and modify for yourself; no redistribution and no commercial
use without written permission; and the licence is revocable on notice.

That is why this repo takes bug reports but not code - see
[CONTRIBUTING.md](CONTRIBUTING.md#code-contributions). The vendored ReShade (BSD 3-Clause) and Dear
ImGui (MIT) headers keep their own permissive licences, which allow this use and ship with every
release.
