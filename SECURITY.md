...
Only the [latest release](https://github.com/Nipeno/MintCap/releases/latest) is
...
[**Report a vulnerability**](https://github.com/Nipeno/MintCap/security/advisories/new).
...
- caps the frame rate, and stores a single number in ReShade's own config;
- opens your browser **only** when you click the GitHub button in the overlay.
...
- **Getting banned on a server.** Every server sets its own rules, and their anticheats may flag
  any injected add-on. That's a policy risk, not a security bug — ask the server's staff if you're
  unsure.
