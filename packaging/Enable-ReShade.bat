@echo off
setlocal
title MintCap - Enable ReShade in FiveM

rem ============================================================================
rem  MintCap - Enable ReShade in FiveM (one-time helper).
rem  Copyright (c) 2026 Nipeno. All rights reserved.
rem  Source available, not open source. See LICENSE.txt in the zip.
rem
...
rem  https://github.com/Nipeno/MintCap
rem ============================================================================
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$m=[char]35+':PS:'+[char]35; $t=[IO.File]::ReadAllText($env:MC_SELF); $i=$t.LastIndexOf($m); Invoke-Expression $t.Substring($i+$m.Length)"
...
Write-Host ''
Write-Host '  MintCap - Enable ReShade in FiveM' -ForegroundColor Green
Write-Host '  ------------------------------------------------'
...
Add-Type -Namespace MC -Name Ini -MemberDefinition @'
...
[void][MC.Ini]::GetPrivateProfileString('Addons', 'ReShade5', '', $sb, 512, $ini)
$current = $sb.ToString()
...
$ok = [MC.Ini]::WritePrivateProfileString('Addons', 'ReShade5', $value, $ini)
if ($ok) {
    Write-Host '  Done! ReShade is now allowed in FiveM.' -ForegroundColor Green
    Write-Host '  Start FiveM, press Home (or Insert with NVE), then set your FPS cap.'
} else {
